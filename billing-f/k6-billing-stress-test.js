...
function invoicePayload() {
  ...
  for (let i = 0; i < itemCount; i++) {
    ...
    const unitPrice = 50000 + Math.floor(Math.random() * 200000);
    const totalPrice = quantity * unitPrice;
    subtotal += totalPrice;
    
    items.push({
      productId: PRODUCT_IDS[Math.floor(Math.random() * PRODUCT_IDS.length)],
      description: `Product Item ${i + 1} - Stress Test`,
      quantity: quantity,
      unitPrice: unitPrice,
...
export function handleSummary(data) {
  ...
  return {
    'billing-stress-summary.json': JSON.stringify({ ...data, k8sRecommendation: recommendation }, null, 2),
  };
}
