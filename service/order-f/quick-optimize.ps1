...
# Check API Gateway
try {
    $response = Invoke-RestMethod -Uri "http://localhost:3000/orders/my?page=1&limit=1" -Method Get -Headers @{
    ...
    if ($_.Exception.Response.StatusCode -eq 401) {
        ...
        Write-Host "  cd nexora-core-services/bmms" -ForegroundColor Gray
        Write-Host "  npm run start api-gateway" -ForegroundColor Gray
        Write-Host "  npm run start order-svc" -ForegroundColor Gray
        Write-Host "  npm run start inventory-svc" -ForegroundColor Gray
    }
}
...
Write-Host "  1. Add database indexes (if not done):" -ForegroundColor White
Write-Host "     cd nexora-core-services/bmms/migrations/order" -ForegroundColor Gray
Write-Host "     mysql -u bmms_user -p order_db < 004_add_performance_indexes.sql" -ForegroundColor Gray
Write-Host ""
Write-Host "  2. Restart order service to apply optimizations:" -ForegroundColor White
Write-Host "     cd nexora-core-services/bmms" -ForegroundColor Gray
Write-Host "     npm run start order-svc" -ForegroundColor Gray
Write-Host ""
Write-Host "  3. Run stress test:" -ForegroundColor White
